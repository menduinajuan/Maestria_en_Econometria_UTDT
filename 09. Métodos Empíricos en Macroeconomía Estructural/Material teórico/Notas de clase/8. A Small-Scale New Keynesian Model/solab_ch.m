...
%
% Modified by C.hevia to be more clear with the blanchard and kahn
% conditions
%
...
% ----------- MODIFICATION BY CONSTANTINO
% Check Blanchard and Kahn conditions. If violated send a message and stop
geiv = abs(diag(t)./diag(s));   % Absolute value of generalized eigenvalues 
ngest = sum(geiv<1);            % Number of stable generalized eigenvalues

if ngest<nk
    error('Blanchard and Kahn condition violated: no existence of equilibrium')
end
if ngest>nk
    error('Blanchard and Kahn condition violated: multiple equilibria. Think for a moment what you want to do with this.')
end
% ----------- END MODIFICATION BY CONSTANTINO:
