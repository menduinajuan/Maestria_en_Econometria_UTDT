%% BVAR of US inflation, unemployment, and interest rate with Minnesota prior
...
subplot(3,1,1)
plot(time,Yraw(:,1),'linestyle','-','linewidth',2,'color',color1)
grid on
ylim([0 16])
title('Inflation')

subplot(3,1,2)
plot(time,Yraw(:,2),'linestyle','-','linewidth',2,'color',color1)
grid on
ylim([0 16])
title('Unemployment')

subplot(3,1,3)
plot(time,Yraw(:,3),'linestyle','-','linewidth',2,'color',color1)
grid on
ylim([0 16])
title('Interest rate')

tightfig;
print(gcf,'Figure_bayesian_us_example_data','-dpdf');
...
do_univariate_autoregressions = 0;      % =1 if compute univariate autoregressions
...
if do_univariate_autoregressions
    ...
    %sigma_sq = diag(OMEGA_OLS);
    sigma_sq = diag( (resid'*resid)/(Traw-p_lags+1) );
end
...
for lag = 1:p_lags
    ...
    for i = 1:n      % For each equation
        for j=1:n    % For each variable in the equation
            ...
            if i==j
                Vi(i,j) = ( lambda1 / lag^lambda3 );
            else
                Vi(i,j) = ( lambda1*lambda2 / lag^lambda3 ) * sigma_sq(i)/sigma_sq(j);
            end
...
for irep = 1:ndraws_total
    ...
    % --------------------------------------------------
    % Draw beta with minnesota prior
    for i=1:n
        %V_post    = inv( inv(V_prior((i-1)*k+1:i*k,(i-1)*k+1:i*k)) + OMEGA_OLS(i,i)\(XX'*XX) );

        % Faster way of computing V_post without using the inv() function in matlab:
        V_pr = V_prior((i-1)*k+1:i*k,(i-1)*k+1:i*k);
        V_post_m1  =  V_pr\speye(size(V_pr)) + OMEGA(i,i)\(XX'*XX);
        V_post = V_post_m1\speye(size(V_post_m1));
        
        beta_post = V_post*( V_prior((i-1)*k+1:i*k,(i-1)*k+1:i*k)\beta_prior((i-1)*k+1:i*k,1) + OMEGA(i,i) \( XX'*YY(:,i))  );
        
        % Draw beta
        beta((i-1)*k+1:i*k,1) = beta_post + chol( V_post )'*randn(k,1);
    end
...
tightfig;
print(gcf,'Figure_us_Minnesota_IR_inflation','-dpdf');
...
tightfig;
print(gcf,'Figure_us_Minnesota_IR_unemployment','-dpdf');
...
tightfig;
print(gcf,'Figure_us_Minnesota_IR_irate','-dpdf');
