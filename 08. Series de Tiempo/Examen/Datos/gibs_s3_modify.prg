...
load data[400,1] = C:\merval_modify.dat;
...
P1_1TT = 0.9;   P1_2TT = 0.05;
P2_1TT = 0.0;   P2_2TT = 0.9;
P3_1TT = 0.1;   P3_2TT = 0.1;

MUTT = 0;
H2TT = 0.9;     H3TT = 2.5;
SIG1TT = 0.001; SIG2TT = SIG1TT * (1 + H2TT);   SIG3TT = SIG2TT * (1 + H3TT);
...
N0 = 100;                   @ Número de muestras a descartar @
MM = 1000;                  @ Número de muestras @
LL = 5;                     @ Cada valor L-TH se usa @
...
@Varianza baja: según el libro @
plotxy(seqa(1,1,rows(s1ttmm)),s1ttmm);
wait;

@Varianza media: según el libro @
plotxy(seqa(1,1,rows(s2ttmm)),s2ttmm);
wait;

@Varianza alta: según el libro @
plotxy(seqa(1,1,rows(s3ttmm)),s3ttmm);
wait;
